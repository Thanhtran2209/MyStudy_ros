...
#include "my_robot_interfaces/srv/reset_counter.hpp"  // include the ResetCounter service definition
...
void callbackResetCounterResponse(rclcpp::Client<ResetCounter>::SharedFuture future)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Success flag: %d  , Message: %s", response->success, response->message.c_str());
}
